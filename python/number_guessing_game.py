...
def Game():
    ...
    d = {
        ...
        "Nice": (69, 420),
        "Leet": (0, 1337),
        "Random": tuple(sorted([random.randint(0, 9999) for _ in range(2)]))
    }
    ...
    clues = [
        f"The number is {len(str(number))} digit{'s' if number>1 else ''} long.",
        f"The number is {(lambda x: 'greater than '+str(x) if number>x else 'less than '+str(x))(random.randint(min_n,max_n))}",
        f"The number is {(lambda x: 'greater than '+str(x) if number>x else 'less than '+str(x))(random.randint(min_n,max_n))}",
        f"The number is {(lambda x: 'greater than '+str(x) if number>x else 'less than '+str(x))(random.randint(min_n,max_n))}",
        f"The number is {'even' if number%2==0 else 'odd'}",
        f"The number is divisible by {random.choice(list(filter(lambda x: (number%x==0 and x!=1), [n for n in range(1,number+1)]))) if number!=0 else [0]}",
        f"The number is divisible by {random.choice(list(filter(lambda x: (number%x==0 and x!=1), [n for n in range(1,number+1)]))) if number!=0 else [0]}",
        f"The number ends with {str(number)[-1]}"
    ]
    ClearScreen()
    while True:
